classdef Body     %
    properties
        ...
        numTriangles; Triangles;TrianglesCoordinates;
    ...
    methods
        ...
        % update every point coordinate  2025/1/29
        function obj = update_mass(obj,disps)
             points = zeros(2,obj.numNodalPoints);
             for p = 1:obj.numNodalPoints
                points(:,p)=obj.NodalPoints(1,p).Coordinates + disps(:,p);
             end
             tris = obj.TrianglesCoordinates;
             h = obj.Thickness;
             for p=1:obj.numTriangles
                 i = tris(p,1); j = tris(p,2); k = tris(p,3);
                 obj.Triangles(p) = obj.Triangles(p).update_mass(points(:,i), points(:,j), points(:,k)); %遍历所有三角形，并记录所有三角形的属性
             end        
        end
