function [time, q] = runSimulation(app)  
    ...
    try
        ...
        f_hydrogel_swelling = @(t,q) dynamic_equation(t,q, app, A,b0,b1, alpha,coor_row);
    
        options = odeset('RelTol',1e-03,'AbsTol',1e-06,'Events', @(time, q) cancelEventFcn(time, q, app));                        
        [time, q] = ode23(f_hydrogel_swelling, interval, qinit,options);
    catch ME
...
function dotq = dynamic_equation(t,q, app, A,b0,b1, alpha, coor_row)
   % 计算进度百分比
    ...
    app.object4D = app.object4D.update_mass(reshape(un, [coor_row,npoints]));
